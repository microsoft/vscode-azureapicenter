...
import { isTenantFilteredOut } from "../../azure/azureAccount/tenant";
...
export async function selectSubscriptions(context: IActionContext, options?: SelectSubscriptionOptions): Promise<void> {
    ...
    if (await provider.isSignedIn()) {
        ...
        ext.treeItem.refresh(context);
    } else {
        const signIn: vscode.MessageItem = { title: UiStrings.SignIn };
        void vscode.window.showInformationMessage(UiStrings.NoSignedIn, signIn).then((input) => {
            if (input === signIn) {
