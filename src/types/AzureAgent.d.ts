...
import { type LocalPluginFunctionResponse, type LocalPluginManifest } from "./AiDriver";
 ...
 */
export type LocalPluginFunctionResponseExtended<T = LocalPluginFunctionResponse> = {
    /**
     * The result of the local plugin function, will be returned to the orchestration platform.
     */
    responseForLanguageModel: T;
...
/**
 * The result type that a local plugin function in VS Code must return.
 *
 * Return values of type {@link LocalPluginFunctionResponse} will be returned to the orchestration platform directly.
 *
 * Return values of type {@link LocalPluginFunctionResponseExtended} will have their fields processed by the agent as described.
 *
 * ❗If the local plugin function has {@link LocalPluginFunction.willHandleUserResponse} set to `true` then this value will be
 * ignored, as the connection to the AI Driver will have been terminated before your function is invoked. Therefore, you can simply
 * return something like an empty `object`, `true`, or empty string, or anything really, to satisfy the type checker.
 */
export type LocalPluginResult = LocalPluginFunctionResponse | LocalPluginFunctionResponseExtended;
...
export type LocalPluginArgs<FunctionNameT = string, ParamsT = { [key: string]: unknown }> = {
    /**
     * The agent request that the local plugin function is being invoked in response to.
     *
     * ❗ If the local plugin function has {@link LocalPluginFunction.willHandleUserResponse} set to `true`, then the
     * {@link AgentRequest.responseStream} can be used to directly respond to the user.
     */
...
export type AgentRequest = {
    ...
    /**
     * The user's attached references (files, terminal text, etc.)
     */
    userReferences: readonly vscode.ChatPromptReference[] | undefined;
