  ...
  public static async reportTestResult(
    ...
    for (const point of points) {
      for (const c of cases) {
        if (
          !c.testPlanCaseId ||
          c.testPlanCaseId !== point.testCaseReference.id
        ) {
          continue;
        }
...
function traverseFolder(path: string, testPoints: TestPoint[]) {
  fs.readdirSync(path).forEach((file) => {
    ...
    if (fs.statSync(currentPath).isDirectory()) {
      ...
      if (currentPath.endsWith("results.json")) {
        console.log("------------------" + currentPath + "------------------");
        const results = fs.readJsonSync(currentPath).suites;
        const cases: TestContext[] = [];

        for (const result of results) {
          for (const testResult of result.specs) {
            if (testResult.tags && testResult.tags.length > 0) {
              testResult.testPlanCaseId = Number(testResult.tags[0])
            }
            const testsItem1 = testResult.tests[0];
            testResult.status = testsItem1.results[0].status
